# Simulation for different sample size and 
# delta (indicates the departure from null hypothesis)
# Author : Salil Koner

# The code requires the value of factor and delta to be specified at first.
# The value of factor needs to be a number between {11, 12, 13, 14, 15, 16, 17, 18, 19, 21} 
# For specific details of what each factor means, please refer to utilities/factor.R. 
# delta indicates the departure from the null hypothesis. See simulation study of the paper. Koner et al.(2021)
...
PVE               <- as.numeric(args[4]) # Percentage of variation explained set to extract the eigenfunctions, should be at least 0.9, and less than 1.
N_Knots           <- as.integer(args[5]) # Number of knots used to represent eta_k(t) using truncated linear basis, should be at least 10;
n_rep             <- as.integer(args[6]) # Number of replication used to empirically compute the power, should be >= 5000 for delta=0;
...
sig.level                 <- c(0.05, 0.1)
power.profit.fourierbasis <- map_dbl(sig.level, ~mean(min_pval.fb < .x/30))
